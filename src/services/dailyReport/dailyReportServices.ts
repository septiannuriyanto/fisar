...
export async function insertFuelmanReport(report: FuelmanDailyReport) {
  try {
    // Step 0: Check if a report with the same date and shift already exists
    ...
    if (checkError && checkError.code !== "PGRST116") {
      // "PGRST116" is Supabase code for "No rows found"
      console.error("❌ Error checking existing report:", checkError);
      throw checkError;
    }

    if (existingReport) {
      const reportIdToDelete = existingReport.id;

      // Delete all child records first to avoid foreign key constraint errors
      const tables = [
        "fuel_issuing_out",
        "fuel_ritasi",
        "fuel_readiness",
      ...
      for (const table of tables) {
        ...
        if (deleteChildError) {
          console.error(`❌ Error deleting from ${table}:`, deleteChildError);
          throw deleteChildError;
        }
      }

      // Then delete the report header
      ...
      if (deleteHeaderError) {
        console.error("❌ Error deleting existing report header:", deleteHeaderError);
        throw deleteHeaderError;
      }

      console.log(`ℹ️ Existing report on ${report.report_date} shift ${report.shift} deleted`);
    }
    ...
    await bulkInsert(
      "fuel_sonding",
      report.sonding.map((item) => ({
        report_id: reportId,
        unit_id: item.unit_id,
        awal: item.awal,
        akhir: item.akhir,
        awal_liter: (item as any).awal_liter ?? null,
